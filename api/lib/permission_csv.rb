...
class PermissionCsv
  ...
  def self.parse_and_create_csv
    ...
    CSV.open(PERMISSION_LIST_FILENAME, 'wb') do |csv|
      policies = Dir.entries(Rails.root.join('app', 'policies')).reject{ |file| %w(. .. admin_policy.rb base_policy.rb provider_policy.rb).include? file }.map{ |file| File.basename(file, '.rb').camelcase }
